...
/**
 * A {@link Completor} implementation that invokes a child completor using the
 * appropriate <i>separator</i> argument. This can be used instead of the
 * individual completors having to know about argument parsing semantics.
 * <p>
 * <strong>Example 1</strong>: Any argument of the command line can use file
 * completion.
 * <p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * <strong>Example 2</strong>: The first argument of the command line can be
 * completed with any of "foo", "bar", or "baz", and remaining arguments can be
 * completed with a file name.
 * <p>
 * 
 * <pre>
 *        consoleReader.addCompletor (new ArgumentCompletor (
 ...
 * completors is always used. To disable this behavior, have the last completor
 * be a {@link NullCompletor}. For example:
 * </p>
 * 
 * <pre>
 *        consoleReader.addCompletor (new ArgumentCompletor (
...
public class ArgumentCompletor implements Completor {
	...
	/**
	 * If true, a completion at argument index N will only succeed if all the
	 * completions from 0-(N-1) also succeed.
	 */
	...
	public int complete(final String buffer, final int cursor, final List<String> candidates) {
		 ...
		 * under the cursor is a delimiter, then trim any delimiters from the
		 * candidates, since we do not need to have an extra delimiter.
		 * 
		 * E.g., if we have a completion for "foo", and we enter "f bar" into
		 * the buffer, and move to after the "f" and hit TAB, we want "foo bar"
		 * instead of "foo  bar".
		 */
	...
	/**
	 * The {@link ArgumentCompletor.ArgumentDelimiter} allows custom breaking up
	 * of a {@link String} into individual arguments in order to dispatch the
	 * arguments to the nested {@link Completor}.
	 * 
	 * @author <a href="mailto:mwp1@cornell.edu">Marc Prud'hommeaux</a>
	 */
	public static interface ArgumentDelimiter {
		/**
		 * Break the specified buffer into individual tokens that can be
		 * completed on their own.
		 * 
		 * @param buffer the buffer to split
		 * @param argumentPosition the current position of the cursor in the
		 *            buffer
		 * @return the tokens
		 */
	...
	/**
	 * Abstract implementation of a delimiter that uses the {@link #isDelimiter}
	...
	public abstract static class AbstractArgumentDelimiter implements ArgumentDelimiter {
		...
		/**
		 * Returns true if the specified character is a whitespace parameter.
		 * Check to ensure that the character is not escaped by any of
		...
		/**
		 * Returns true if the character at the specified position if a
		 * delimiter. This method will only be called if the character is not
		 * enclosed in any of the {@link #getQuoteChars}, and is not escaped by
		 * ant of the {@link #getEscapeChars}. To perform escaping manually,
	...
	/**
	 * {@link ArgumentCompletor.ArgumentDelimiter} implementation that counts
	 * all whitespace (as reported by {@link Character#isWhitespace}) as being a
	 * delimiter.
	 * 
	 * @author <a href="mailto:mwp1@cornell.edu">Marc Prud'hommeaux</a>
	 */
	public static class WhitespaceArgumentDelimiter extends AbstractArgumentDelimiter {
		/**
		 * The character is a delimiter if it is whitespace, and the preceeding
	...
	public static class ArgumentList {
		 ...
		 * @param argumentPosition the position of the cursor in the current
		 *            token
