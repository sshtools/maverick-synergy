...
public class S3AbstractFile implements S3File {
	...
	public MultipartTransfer startMultipartUpload(Collection<Multipart> multiparts) throws IOException, PermissionDeniedException {
        ...
        return new S3MultipartTransfer(key, this, response.uploadId(), multiparts, exists());
	}
...
//			executor = Executors.newCachedThreadPool();
		...
		public void closePart(Multipart part) throws IOException {
			...
			if(Log.isErrorEnabled()) {
				Log.info("REMOVEME: Closing part {} {}/{} with id {} {} {}", part.getPartIdentifier(),
						bucket.getName(), path, getUploadId(), completedParts.size(), multiparts.size());
			}
		...
		private void cancelUpload() {
			try {
				if(Log.isErrorEnabled()) {
					Log.info("REMOVEME: Cancelling upload {}/{} with id {}", bucket.getName(), path, getUploadId());
				}
...
//			}
			...
			try {
				...
				if(Objects.nonNull(completeCallback)) {
					completeCallback.multipartCompleted(this);
				}
