...
public class ConsoleReader implements ConsoleOperations {
	...
	static {
		...
		Map codes = new TreeMap();

		putKeyName(names,codes,"MOVE_TO_BEG", new Short(MOVE_TO_BEG));
		putKeyName(names,codes,"MOVE_TO_END", new Short(MOVE_TO_END));
		putKeyName(names,codes,"PREV_CHAR", new Short(PREV_CHAR));
		putKeyName(names,codes,"NEWLINE", new Short(NEWLINE));
		putKeyName(names,codes,"KILL_LINE", new Short(KILL_LINE));
		putKeyName(names,codes,"PASTE", new Short(PASTE));
		putKeyName(names,codes,"CLEAR_SCREEN", new Short(CLEAR_SCREEN));
		putKeyName(names,codes,"NEXT_HISTORY", new Short(NEXT_HISTORY));
		putKeyName(names,codes,"PREV_HISTORY", new Short(PREV_HISTORY));
		putKeyName(names,codes,"START_OF_HISTORY", new Short(START_OF_HISTORY));
		putKeyName(names,codes,"END_OF_HISTORY", new Short(END_OF_HISTORY));
		putKeyName(names,codes,"REDISPLAY", new Short(REDISPLAY));
		putKeyName(names,codes,"KILL_LINE_PREV", new Short(KILL_LINE_PREV));
		putKeyName(names,codes,"DELETE_PREV_WORD", new Short(DELETE_PREV_WORD));
		putKeyName(names,codes,"NEXT_CHAR", new Short(NEXT_CHAR));
		putKeyName(names,codes,"REPEAT_PREV_CHAR", new Short(REPEAT_PREV_CHAR));
		putKeyName(names,codes,"SEARCH_PREV", new Short(SEARCH_PREV));
		putKeyName(names,codes,"REPEAT_NEXT_CHAR", new Short(REPEAT_NEXT_CHAR));
		putKeyName(names,codes,"SEARCH_NEXT", new Short(SEARCH_NEXT));
		putKeyName(names,codes,"PREV_SPACE_WORD", new Short(PREV_SPACE_WORD));
		putKeyName(names,codes,"TO_END_WORD", new Short(TO_END_WORD));
		putKeyName(names,codes,"REPEAT_SEARCH_PREV", new Short(REPEAT_SEARCH_PREV));
		putKeyName(names,codes,"PASTE_PREV", new Short(PASTE_PREV));
		putKeyName(names,codes,"REPLACE_MODE", new Short(REPLACE_MODE));
		putKeyName(names,codes,"SUBSTITUTE_LINE", new Short(SUBSTITUTE_LINE));
		putKeyName(names,codes,"TO_PREV_CHAR", new Short(TO_PREV_CHAR));
		putKeyName(names,codes,"NEXT_SPACE_WORD", new Short(NEXT_SPACE_WORD));
		putKeyName(names,codes,"DELETE_PREV_CHAR", new Short(DELETE_PREV_CHAR));
		putKeyName(names,codes,"ADD", new Short(ADD));
		putKeyName(names,codes,"PREV_WORD", new Short(PREV_WORD));
		putKeyName(names,codes,"CHANGE_META", new Short(CHANGE_META));
		putKeyName(names,codes,"DELETE_META", new Short(DELETE_META));
		putKeyName(names,codes,"END_WORD", new Short(END_WORD));
		putKeyName(names,codes,"NEXT_CHAR", new Short(NEXT_CHAR));
		putKeyName(names,codes,"INSERT", new Short(INSERT));
		putKeyName(names,codes,"REPEAT_SEARCH_NEXT", new Short(REPEAT_SEARCH_NEXT));
		putKeyName(names,codes,"PASTE_NEXT", new Short(PASTE_NEXT));
		putKeyName(names,codes,"REPLACE_CHAR", new Short(REPLACE_CHAR));
		putKeyName(names,codes,"SUBSTITUTE_CHAR", new Short(SUBSTITUTE_CHAR));
		putKeyName(names,codes,"TO_NEXT_CHAR", new Short(TO_NEXT_CHAR));
		putKeyName(names,codes,"UNDO", new Short(UNDO));
		putKeyName(names,codes,"NEXT_WORD", new Short(NEXT_WORD));
		putKeyName(names,codes,"DELETE_NEXT_CHAR", new Short(DELETE_NEXT_CHAR));
		putKeyName(names,codes,"CHANGE_CASE", new Short(CHANGE_CASE));
		putKeyName(names,codes,"COMPLETE", new Short(COMPLETE));
		putKeyName(names,codes,"EXIT", new Short(EXIT));
		putKeyName(names,codes,"CLEAR_LINE", new Short(CLEAR_LINE));
		putKeyName(names,codes,"INTERRUPT", new Short(INTERRUPT));

		KEYMAP_NAMES = new TreeMap(Collections.unmodifiableMap(names));
		KEYMAP_CODES = new TreeMap(Collections.unmodifiableMap(codes));
	}
	
	static void putKeyName(Map names, Map codes, String key, Short value) {
		names.put(key, value);
		codes.put(value, key);
	}
	 ...
	 */
	private int autoprintThreshhold = Integer.getInteger("jline.completion.threshold", 100).intValue(); // same
																										// default
																										// as
	 ...
	 * addTriggerAction('q', new ActionListener(){ System.exit(0); }); would do
	 * the trick.
	 * 
	 * @param c
	 * @param listener
	...
	public ConsoleReader() throws IOException {
		this(new FileInputStream(FileDescriptor.in), new SystemOutputStream(System.out));
	}
	...
	public ConsoleReader(final InputStream in, final OutputStream out, final InputStream bindings) throws IOException {
	...
	public void setKeyBindings(InputStream bindings) throws IOException {
		this.keybindings = new HashMap<String,Short>();
			...
			for (Iterator<Object> i = p.keySet().iterator(); i.hasNext();) {
				...
				try {
					...
					if (opval != null) {
						...
						keybindings.put(buf.toString(), Short.valueOf(opval.shortValue()));
					}
	...
	/**
	 * @param autoprintThreshhold the number of candidates to print without
	 *            issuing a warning.
	 */
	...
	public String getKeyForAction(short logicalAction) {
		for (Iterator<Entry<String,Short>> e = keybindings.entrySet().iterator(); e.hasNext();) {
			Map.Entry<String,Short> entry = e.next();
			if (((Short) entry.getValue()).shortValue() == logicalAction) {
	...
	public String readLine(final String prompt, final Character mask) throws IOException {
		...
		try {
			...
			while (true) {
				Object[] next = readBinding();

				if (next == null) {
					return null;
				}

				String c = (String) next[0];
				int code = next[1] == null ? 0 : ((Short) next[1]).intValue();
				...
				case UNKNOWN:
				default:
					if (c != null) { // ignore null chars
						for (int i = 0; i < c.length(); i++) {
							char cc = c.charAt(i);
							if (cc != 0) {
								ActionListener action = (ActionListener) triggeredActions.get(new Character((char) cc));
								if (action != null)
	...
	public Object[] readBinding() throws IOException {
		...
		while (true) {
			...
			if (keybindings.containsKey(key)) {
				// Exact match
				short code = ((Short) keybindings.get(key)).shortValue();
				if (debugger != null) {
					// TODO print values
					debug("    translated: " + key + ": " + code);
				}
				if (code == INTERRUPT) {
					Thread.currentThread().interrupt();
				}
				return new Object[] { key, new Short(code) };
			}

			// Do any bindings even start with this code?
			boolean found = false;
			for (Iterator<Entry<String, Short>> i = keybindings.entrySet().iterator(); i.hasNext();) {
				Map.Entry<String,Short> ent = i.next();
				if (ent.getKey().toString().startsWith(key)) {
					// Something does start with this code, so loop around and
	...
	public boolean paste() throws IOException {
		...
		try {
			 ...
			 * /index.php?func=detail&aid=1060649&group_id=64033&atid=506056 to
			 * get around the deprecated DataFlavor.plainTextFlavor, but it
...
//			System.out.println("Doing completor " + comp);
	...
	public void printColumns(final Collection<String> stuff) throws IOException {
		...
		for (Iterator<String> i = stuff.iterator(); i.hasNext(); maxwidth = Math.max(maxwidth, i.next().toString().length())) {
			;
		}
	...
	/**
	 * Append <i>toPad</i> to the specified <i>appendTo</i>, as well as
	 * (<i>toPad.length () - len</i>) spaces.
	 * 
	...
	public void printException(Throwable exception) throws IOException {
		...
		printStringNewline(sw.toString());		
	}
	 ...
	 */
	public final void printStringNewline(final String str) throws IOException {
		printString(str);
		printNewline();
	}
	 ...
	 */
	public final void putChar(final int c, final boolean print) throws IOException {
		buf.write((char) c);
	 ...
	 * 
	 * @param clear the number of characters to clear after the end of the
	 *            buffer
	 */
	 ...
	 * the current buffer.
	 */
	public final void printCharacter(final int c) throws IOException {
		if (c == '\t') {
	 ...
	 * the current buffer.
	 */
	public final void printCharacters(final char[] c) throws IOException {
		int len = 0;
		for (int i = 0; i < c.length; i++)
			if (c[i] == '\t')
				len += TAB_WIDTH;
	...
	private final int delete(final int num) throws IOException {
		/*
		 * Commented out beacuse of DWA-2949: if (buf.cursor == 0) return 0;
		 */
	 ...
	 * </p>
	 * 
	 * @param echoCharacter the character to echo to the console in place of the
	 *            typed character.
	 */
	 ...
	 * 
	 * @param eol eol sequence
	 */
	public final void setEOL(int eol) {
		this.eol = eol;
	}

	private static int getDefaultEOL() {
	...
	static class SystemOutputStream extends OutputStream {
		...
		SystemOutputStream(PrintStream out) {
			this.out = out;
		}
