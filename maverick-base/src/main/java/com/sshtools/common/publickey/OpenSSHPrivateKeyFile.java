...
/* (non-Javadoc)
  ...
  private void writeOpenSSHKeyPair(ByteArrayWriter writer, SshKeyPair pair, String passphrase) throws IOException, NoSuchAlgorithmException, SshException {
	  ...
	  ComponentFactory<OpenSSHPrivateKeyFileParser> encoders = new ComponentFactory<>(JCEComponentManager.getDefaultInstance());
	  JCEComponentManager.getDefaultInstance().loadExternalComponents("openssh-parser.properties", encoders);
	  ...
	  try(ByteArrayWriter options = new ByteArrayWriter()) {
		  ...
		  try(ByteArrayWriter privateKeyData = new ByteArrayWriter()) {
			  ...
			  switch(pair.getPublicKey().getAlgorithm()) {
			  case "ssh-rsa":
			  {
				  ...
				  privateKeyData.writeBigInteger(privatekey.getPublicExponent());
		      ...
		      default:
		    	  ...
		    	  OpenSSHPrivateKeyFileParser encoder = encoders.getInstance(pair.getPublicKey().getAlgorithm());
		    	  encoder.encode(privateKeyData, pair);
		    	  break;
			  }
			  ...
			  if(!cipherName.equals("none")) {
				  ...
				  byte[] keydata = BCryptKDF.bcrypt_pbkdf(passphrase.getBytes("UTF-8"), 
						  salt, iv.length + key.length, rounds);
			  ...
			  try {
				  ...
				  for(int i=0;i<count;i++) {
				      ...
				      default:
				    	  ...
				    	  OpenSSHPrivateKeyFileParser encoder = encoders.getInstance(pair.getPublicKey().getAlgorithm());
				    	  encoder.decode(privateReader, pair);
				    	  break;
					  }
...
//      PEMReader pem = 
  ...
  public static void main(String[] args) throws IOException, InvalidPassphraseException, SshException {
	  
	  try(InputStream in = new FileInputStream("/Users/lee/Downloads/ed25519_Lee")) {
